...
for course,longName in slidesToGenerate:
    for unit in range(10):
        for day in range(10):
            dayTemplateName = ('courses/'+course
                                +'/Unit'+str(unit)
                                +'/Day'+str(day)
                                +'/content.md')
            generatedMdFileName = ('generated/markdown/'+course
                                   +'/Unit'+str(unit)
                                   +'/Day'+str(day)+'.md')
            generatedHtmlFileName = ('generated/html/'+course+'/'
                                     +course
                                     +'_Unit'+str(unit)
                                     +'_Day'+str(day)+'.html')
            try:
            ...
            output_from_parsed_template = template.render(
                                            unit_and_day='Unit ' + str(unit) + ', Day ' + str(day),
                                            course = longName)
            ...
            pypandoc.convert_file(generatedMdFileName
                                  , to='revealjs'
                                  , outputfile=generatedHtmlFileName
                                  , extra_args=pdoc_args)
            
            #Post slides to website
            path = ('/var/www/whscs.net/html/courses/' 
                + course 
                + '/Unit'+str(unit) + '/'
                +  course
                +'_Unit'+str(unit)
...
if 'other' in sys.argv:
    ...
    for root, dirs, files in os.walk("./templates/courses/Other"):
        for name in files:
            ...
            generatedHtmlFileName = ('generated/html/Other/'
            ...
            #Post slides to website
            path = ('/var/www/whscs.net/html/courses/Other/'
                +  name.replace('.md','.html'))
            localpath = generatedHtmlFileName
            #print("uploading " + generatedHtmlFileName)
            #print("remote filename will be " + path)
...
if 'img' in sys.argv:
    ...
    for root, dirs, files in os.walk("./templates/images"):
        for name in files:
            ...
            remotepath = ('/var/www/whscs.net/html/courses/images/' 
                        + name)
